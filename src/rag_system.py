...
class GreenDreamRAG:
    ...
    def _format_revista(self, revista: Dict) -> str:
        ...
        contenido_destacado = ' '.join([f'- {item}' for item in revista['contenido_destacado']])
        content = f"""
    ...
    def get_recommendations_context(self, query: str) -> str:
        ...
        if not relevant_docs:
            ...
            summary_lines = ["🌱 **Resumen breve de la Base de Conocimiento Green Dream:**\n"]
            # Mostrar hasta 5 títulos representativos
            for i, doc in enumerate(self.documents[:5], 1):
                meta = doc.get('metadata', {})
                title = meta.get('titulo') or meta.get('nombre') or doc.get('source', f'Doc {i}')
